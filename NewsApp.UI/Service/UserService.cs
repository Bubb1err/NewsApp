...
public class UserService
{
    ...
    public async Task<List<Guid>> GetUserLikes()
    {
            ...
            var response = await _httpClient.GetFromJsonAsync<DataCollectionApiResponseDto<Guid>>("http://localhost:5296/api/User/likes");
    ...
    public async Task<List<Guid>> GetUserBookmarks()
    {
        var token = await _tokenProvider.GetTokenAsync();
        _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
        var response = await _httpClient.GetFromJsonAsync<DataCollectionApiResponseDto<Guid>>("http://localhost:5296/api/User/bookmarks");
        return response.Items;
